...
func (e *EventDispatcherManager) Init() error {
	...
	workerCount := 1
	e.sink = sink.NewMysqlSink(e.changefeedID, workerCount, cfg, db)
	return nil
	//}
...
func (e *EventDispatcherManager) NewTableEventDispatcher(tableSpan *common.TableSpan, startTs uint64) *dispatcher.TableEventDispatcher {
	...
	*/
	...
	log.Info("new table event dispatcher created", zap.Any("dispatcherID", tableEventDispatcher.GetId()), zap.Any("tableSpan", tableSpan), zap.Any("changefeedID", e.changefeedID.String()),
