...
// WriteFloat32Field writes a float32 field like "<fieldName>":<value>.
func (w *JSONWriter) WriteFloat32Field(fieldName string, value float32) {
	if w.needPrependComma {
		w.stream.WriteMore()
	} else {
		w.needPrependComma = true
