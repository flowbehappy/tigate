...
func (m *mockEventStore) SubscribeTableSpan(span *common.TableSpan, startTs uint64, onSpanUpdate func(watermark uint64)) (uint64, error) {
...
func (m *mockEventStore) Read(dataRange ...*common.DataRange) ([][]*common.TxnEvent, error) {
...
func TestEventServiceBasic(t *testing.T) {
	...
	es := NewEventService(ctx, mc, eventStore)
	...
	sourceSpanStat, ok := eventStore.spans[acceptorInfo.span.TableID]
	require.True(t, ok)
...
func TestDispatcherCommunicateWithEventService(t *testing.T) {
	...
	defer cancel()
	eventStore := newMockeventStore()
	eventService := NewEventService(ctx, appcontext.GetService[messaging.MessageCenter](appcontext.MessageCenter), eventStore)
	...
	sourceSpanStat, ok := eventStore.spans[tableSpan.TableID]
	require.True(t, ok)
