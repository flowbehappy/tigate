...
func VerifyChecksum(event *commonEvent.RowChangedEvent, db *sql.DB) error {
	...
	if event.Checksum.Current != 0 {
		checksum, err := calculateChecksum(event.Columns, event.TableInfo.ColumnSchema.Columns)
		if err != nil {
	...
	if event.Checksum.Previous != 0 {
		checksum, err := calculateChecksum(event.PreColumns, event.TableInfo.ColumnSchema.Columns)
		if err != nil {
			return errors.Trace(err)
		}
