...
type BatchDecoder struct {
	...
	nextEvent *common.RowChangedEvent
...
func (b *BatchDecoder) buildColumns(
	holder *ticommon.ColumnsHolder, handleKeyColumns map[string]interface{},
) []*common.Column {
	columnsCount := holder.Length()
	columns := make([]*common.Column, 0, columnsCount)
	for i := 0; i < columnsCount; i++ {
		...
		case mysql.TypeBit:
			value = ticommon.MustBinaryLiteralToInt(value.([]uint8))
		}
		...
		if _, ok := handleKeyColumns[name]; ok {
			column.Flag = common.PrimaryKeyFlag | common.HandleKeyFlag
		}
...
func (b *BatchDecoder) assembleHandleKeyOnlyEvent(
	ctx context.Context, handleKeyOnlyEvent *common.RowChangedEvent,
	...
	if handleKeyOnlyEvent.IsInsert() {
		...
		holder := ticommon.MustSnapshotQuery(ctx, b.upstreamTiDB, commitTs, schema, table, conditions)
		columns := b.buildColumns(holder, conditions)
		indexColumns := common.GetHandleAndUniqueIndexOffsets4Test(columns)
		handleKeyOnlyEvent.TableInfo = common.BuildTableInfo(schema, table, columns, indexColumns)
		...
		holder := ticommon.MustSnapshotQuery(ctx, b.upstreamTiDB, commitTs-1, schema, table, conditions)
		preColumns := b.buildColumns(holder, conditions)
		indexColumns := common.GetHandleAndUniqueIndexOffsets4Test(preColumns)
		handleKeyOnlyEvent.TableInfo = common.BuildTableInfo(schema, table, preColumns, indexColumns)
...
func (b *BatchDecoder) assembleEventFromClaimCheckStorage(ctx context.Context) (*common.RowChangedEvent, error) {
