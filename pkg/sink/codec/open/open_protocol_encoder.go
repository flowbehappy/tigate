...
func (d *BatchEncoder) buildMessageOnlyHandleKeyColumns(e *common.RowChangedEvent) ([]byte, []byte, error) {
...
func (d *BatchEncoder) EncodeDDLEvent(e *model.DDLEvent) (*ticommon.Message, error) {
	...
	ret := ticommon.NewDDLMsg(config.ProtocolOpen, keyBuf.Bytes(), valueBuf.Bytes(), e)
	return ret, nil
}
...
func (d *BatchEncoder) EncodeCheckpointEvent(ts uint64) (*ticommon.Message, error) {
	...
	ret := ticommon.NewResolvedMsg(config.ProtocolOpen, keyBuf.Bytes(), valueBuf.Bytes(), ts)
	return ret, nil
}
...
func (d *BatchEncoder) Build() (messages []*ticommon.Message) {
	d.tryBuildCallback()
	ret := d.messageBuf
	d.messageBuf = make([]*ticommon.Message, 0)
	return ret
}
...
func (d *BatchEncoder) newClaimCheckLocationMessage(
	event *common.RowChangedEvent, fileName string,
) ([]byte, []byte, error) {
