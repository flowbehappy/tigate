...
func rowChangeToMaxwellMsg(e *common.RowChangedEvent, onlyHandleKeyColumns bool) (*internal.MessageKey, *maxwellMessage) {
	var partition *int64
	...
	if e.IsDelete() {
		...
		for _, v := range e.PreColumns {
			...
			id := tableInfo.ForceGetColumnIDByName(v.Name)
			colInfo := tableInfo.ForceGetColumnInfo(id)
			colName := tableInfo.ForceGetColumnName(id)
		...
		for _, v := range e.Columns {
			colFlag := v.Flag
			colId := tableInfo.ForceGetColumnIDByName(v.Name)
