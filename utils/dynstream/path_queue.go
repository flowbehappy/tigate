package dynstream

import (
	"time"

	"github.com/flowbehappy/tigate/utils/heap"
)

// The path node order by timestamp.
type tsPathNode[A Area, P Path, T Event, D Dest, H Handler[A, P, T, D]] pathInfo[A, P, T, D, H]

func (n *tsPathNode[A, P, T, D, H]) c(index int) {
	(*pathInfo[A, P, T, D, H])(n).tsHeapIndex = index
}

func (n *tsPathNode[A, P, T, D, H]) SetHeapIndex(index int) {
	(*pathInfo[A, P, T, D, H])(n).tsHeapIndex = index
}
func (n *tsPathNode[A, P, T, D, H]) GetHeapIndex() int {
	return (*pathInfo[A, P, T, D, H])(n).tsHeapIndex
}

func (n *tsPathNode[A, P, T, D, H]) LessThan(other *tsPathNode[A, P, T, D, H]) bool {
	q1 := (*pathInfo[A, P, T, D, H])(n).pendingQueue
	q2 := (*pathInfo[A, P, T, D, H])(other).pendingQueue
	f1, _ := q1.FrontRef()
	f2, _ := q2.FrontRef()
	return f1.timestamp < f2.timestamp
}

// The path node order by queue time.
type qtPathNode[A Area, P Path, T Event, D Dest, H Handler[A, P, T, D]] pathInfo[A, P, T, D, H]

func (n *qtPathNode[A, P, T, D, H]) SetHeapIndex(index int) {
	(*pathInfo[A, P, T, D, H])(n).qtHeapIndex = index
}
func (n *qtPathNode[A, P, T, D, H]) GetHeapIndex() int {
	return (*pathInfo[A, P, T, D, H])(n).qtHeapIndex
}
func (n *qtPathNode[A, P, T, D, H]) LessThan(other *qtPathNode[A, P, T, D, H]) bool {
	q1 := (*pathInfo[A, P, T, D, H])(n).pendingQueue
	q2 := (*pathInfo[A, P, T, D, H])(other).pendingQueue
	f1, _ := q1.FrontRef()
	f2, _ := q2.FrontRef()
	return f1.queueTime.Before(f2.queueTime)
}

// A area info contains the path nodes of the area in a stream.
// Note that the instance is stream level, not global level.
type areaInfo[A Area, P Path, T Event, D Dest, H Handler[A, P, T, D]] struct {
	area A

	// The path bound to the area info instance.
	// Since tsHeap and qtHeap only store the paths who has pending events,
	// the pathCount could be larger than the length of the heaps.
	pathCount int

	tsHeap heap.Heap[*tsPathNode[A, P, T, D, H]]
	qtHeap heap.Heap[*qtPathNode[A, P, T, D, H]]

	qtHeapIndex int
}

func (a *areaInfo[A, P, T, D, H]) minQueueTime() time.Time {
	top, _ := a.qtHeap.PeekTop()
	front, _ := top.pendingQueue.FrontRef()
	return front.queueTime
}

func (a *areaInfo[A, P, T, D, H]) SetHeapIndex(index int) {
	a.qtHeapIndex = index
}

func (a *areaInfo[A, P, T, D, H]) GetHeapIndex() int {
	return a.qtHeapIndex
}

func (a *areaInfo[A, P, T, D, H]) LessThan(other *areaInfo[A, P, T, D, H]) bool {
	return a.minQueueTime().Before(other.minQueueTime())
}

type pathQueue[A Area, P Path, T Event, D Dest, H Handler[A, P, T, D]] struct {
	option  Option
	handler H

	areaMap  map[A]*areaInfo[A, P, T, D, H]
	areaHeap heap.Heap[*areaInfo[A, P, T, D, H]]

	totalPendingLength int
}

func newPathQueue[A Area, P Path, T Event, D Dest, H Handler[A, P, T, D]](option Option, handler H) pathQueue[A, P, T, D, H] {
	return pathQueue[A, P, T, D, H]{
		option:   option,
		areaMap:  make(map[A]*areaInfo[A, P, T, D, H]),
		areaHeap: heap.NewHeap[*areaInfo[A, P, T, D, H]](),
	}
}

func (q *pathQueue[A, P, T, D, H]) updateHeapAfterUpdatePath(path *pathInfo[A, P, T, D, H]) {
	area := path.areaInfo

	if path.removed || path.blocking || path.pendingQueue.Length() == 0 {
		// Remove the path from heap
		area.tsHeap.Remove((*tsPathNode[A, P, T, D, H])(path))
		area.qtHeap.Remove((*qtPathNode[A, P, T, D, H])(path))

		if area.qtHeap.Len() == 0 {
			q.areaHeap.Remove(area)
		} else {
			q.areaHeap.AddOrUpdate(area)
		}
	} else {
		area.tsHeap.AddOrUpdate((*tsPathNode[A, P, T, D, H])(path))
		area.qtHeap.AddOrUpdate((*qtPathNode[A, P, T, D, H])(path))
		q.areaHeap.AddOrUpdate(area)
	}
}

func (q *pathQueue[A, P, T, D, H]) addPath(path *pathInfo[A, P, T, D, H]) {
	area, ok := q.areaMap[path.area]
	if !ok {
		area = &areaInfo[A, P, T, D, H]{
			area:   path.area,
			tsHeap: heap.NewHeap[*tsPathNode[A, P, T, D, H]](),
			qtHeap: heap.NewHeap[*qtPathNode[A, P, T, D, H]](),
		}
		q.areaMap[path.area] = area
	}

	area.pathCount++
	path.areaInfo = area
	path.qtHeapIndex = 0
	path.tsHeapIndex = 0

	q.totalPendingLength += path.pendingQueue.Length()
	q.updateHeapAfterUpdatePath(path)
}

func (q *pathQueue[A, P, T, D, H]) removePath(path *pathInfo[A, P, T, D, H]) {
	area := path.areaInfo
	if area == nil {
		return
	}
	area.pathCount--

	if area.pathCount == 0 {
		delete(q.areaMap, area.area)
	}

	q.totalPendingLength -= path.pendingQueue.Length()
	q.updateHeapAfterUpdatePath(path)
	path.areaInfo = nil
}

func (q *pathQueue[A, P, T, D, H]) appendEvent(event eventWrap[A, P, T, D, H]) {
	path := event.pathInfo
	if path.areaInfo == nil {
		// A newly added path sends the first event.
		q.addPath(path)
	}

	if q.option.MaxPendingLength > 0 && path.pendingQueue.Length() >= q.option.MaxPendingLength {
		switch q.option.DropPolicy {
		case DropLate:
			q.handler.OnDrop(event.event)
		case DropEarly:
			dropped, _ := path.pendingQueue.PopFront()
			q.handler.OnDrop(dropped.event)
			event.pathInfo.pendingQueue.PushBack(event)
		}
	} else {
		event.pathInfo.pendingQueue.PushBack(event)
		q.totalPendingLength++
	}

	q.updateHeapAfterUpdatePath(path)
}

func (q *pathQueue[A, P, T, D, H]) popEvents(buf []T) ([]T, *pathInfo[A, P, T, D, H]) {
	batchSize := q.option.BatchCount
	if batchSize == 0 {
		batchSize = 1
	}
	for {
		area, ok := q.areaHeap.PeekTop()
		if !ok {
			return buf[:0], nil
		}
		top, ok := area.tsHeap.PeekTop()
		if !ok {
			panic("top is nil")
		}
		path := (*pathInfo[A, P, T, D, H])(top)
		if path.removed {
			q.updateHeapAfterUpdatePath(path)
			continue
		} else {
			var eType EventType = 0
			for i := 0; i < batchSize; i++ {
				front, ok := path.pendingQueue.FrontRef()
				if !ok || (eType != 0 && eType != front.eventType) {
					break
				}
				eType = front.eventType
				buf = append(buf, front.event)
				path.pendingQueue.PopFront()
			}
			if len(buf) == 0 {
				panic("empty buf")
			}

			q.totalPendingLength -= len(buf)
			q.updateHeapAfterUpdatePath((*pathInfo[A, P, T, D, H])(path))
			return buf, path
		}
	}
}

func (q *pathQueue[A, P, T, D, H]) blockPath(path *pathInfo[A, P, T, D, H]) {
	q.updateHeapAfterUpdatePath((*pathInfo[A, P, T, D, H])(path))
}

func (q *pathQueue[A, P, T, D, H]) wakePath(path *pathInfo[A, P, T, D, H]) {
	q.updateHeapAfterUpdatePath((*pathInfo[A, P, T, D, H])(path))
}
