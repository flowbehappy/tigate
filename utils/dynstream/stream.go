...
func (s *stream[A, P, T, D, H]) handleLoop(acceptedPaths []*pathInfo[A, P, T, D, H], formerStreams []*stream[A, P, T, D, H]) {
	...
	defer func() {
		...
		// Move remaining events in the outChan to pendingQueue.
		for e := range s.outChan {
				...
				if path.blocking {
					...
					if minTS > 0 {
						log.Info("_statMinHandledTS store",
							zap.Uint64("minTS", uint64(minTS)), zap.Uint64("inputChannelSize", uint64(s.option.InputBufferSize)))
